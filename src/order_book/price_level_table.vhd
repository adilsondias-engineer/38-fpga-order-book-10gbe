...
--   LOOKUP_LEVEL  : Read price level data (3-cycle latency)
...
architecture Behavioral of price_level_table is
    ...
    -- Stage 1 registers (cycle 1 after cmd_valid)
    ...
    -- Stage 2 registers (cycle 2 after cmd_valid - bram_do now valid)
    -- CRITICAL FIX: pipe_old_level must be captured when bram_do is valid,
    -- which is one cycle AFTER pipe_cmd_valid is set.
    signal pipe_cmd_valid_d1 : std_logic := '0';  -- Delay 1: for capturing pipe_old_level
    ...
    -- Stage 3 registers (cycle 3 after cmd_valid - pipe_old_level now valid)
    signal pipe_cmd_valid_d2 : std_logic := '0';  -- Delay 2: for Stage 2 processing
    ...
    -- Pipeline timing:
    --   Cycle 0: cmd_valid=1, bram_addr set
    --   Cycle 1: bram_do has OLD data, pipe_cmd_valid=1
    --   Cycle 2: bram_do has CORRECT data, pipe_lookup_d1=1 (capture here!)
    --   Cycle 3: rd_valid_stage2=1, data available to consumer
    signal pipe_lookup_d1 : std_logic := '0';  -- Delayed LOOKUP flag to capture correct bram_do
    ...
    attribute MAX_FANOUT of bid_count : signal is 16;
    attribute MAX_FANOUT of ask_count : signal is 16;
    attribute MAX_FANOUT of pipe_cmd_shares_d2 : signal is 16;
    attribute MAX_FANOUT of pipe_cmd_side_d2 : signal is 16;
    attribute MAX_FANOUT of pipe_old_level : signal is 16;
    attribute MAX_FANOUT of pipe_cmd_valid_d2 : signal is 16;
    attribute MAX_FANOUT of pipe_cmd_addr_d2 : signal is 16;
    attribute MAX_FANOUT of pipe_cmd_price_d2 : signal is 16;
    attribute MAX_FANOUT of pipe_cmd_type_d2 : signal is 16;
    attribute MAX_FANOUT of bram_di : signal is 16;
    attribute MAX_FANOUT of bram_we : signal is 16;
    attribute MAX_FANOUT of bram_addr : signal is 16;
    attribute MAX_FANOUT of bram_do : signal is 16;
    attribute MAX_FANOUT of bram : signal is 16;
...
begin
    ...
    -- Pipeline timing for ADD/REMOVE:
    --   Cycle 0: cmd_valid=1, bram_addr set in Stage 2
    --   Cycle 1: pipe_cmd_valid=1, bram_do=OLD (wrong!)
    --   Cycle 2: pipe_cmd_valid_d1=1, bram_do=CORRECT, capture pipe_old_level
    --   Cycle 3: pipe_cmd_valid_d1=0, Stage 2 processes with correct pipe_old_level
    ------------------------------------------------------------------------
    process(clk)
    begin
        if rising_edge(clk) then
            if rst = '1' then
                ...
                -- Delay pipeline by one cycle (Stage 1 -> Stage 2)
                -- This ensures bram_do has valid data when pipe_old_level is captured
                ...
                -- Delay pipeline by another cycle (Stage 2 -> Stage 3)
                -- This ensures pipe_old_level is valid when Stage 2 processes
                ...
                -- Capture old level data from BRAM output
                -- CRITICAL FIX: Capture when pipe_cmd_valid_d1=1 (not pipe_cmd_valid=1)
                -- At this point, bram_do contains the correct data for the requested address
    ...
    -- CRITICAL FIX: Use _d2 pipeline registers for ADD/REMOVE processing
    -- Pipeline timing:
    --   Cycle 0: cmd_valid=1, bram_addr set
    --   Cycle 1: pipe_cmd_valid=1, bram_do=OLD (wrong!)
    --   Cycle 2: pipe_cmd_valid_d1=1, bram_do=CORRECT, capture pipe_old_level
    --   Cycle 3: pipe_cmd_valid_d2=1, pipe_old_level valid, process here!
    ------------------------------------------------------------------------
    process(clk)
        ...
        if rising_edge(clk) then
            if rst = '1' then
                ...
                if cmd_valid = '1' and cmd_type /= CMD_CLEAR then
                    ...
                    -- Cycle 3: Use _d2 pipelined address (for write operations)
                ...
                if cmd_valid = '1' and cmd_type = CMD_CLEAR then
                    ...
                    bram_addr <= cmd_addr;
                    bram_di <= (others => '0');
                -- Stage 2: Process read-modify-write operations using _d2 registers
    ...
    ------------------------------------------------------------------------
    -- Read Output Pipeline (for LOOKUP commands)
    -- 3-cycle pipeline: cmd -> bram_addr -> bram_do valid -> capture -> output
    --
    -- CRITICAL FIX: bram_do is available ONE cycle AFTER bram_addr is set.
    -- When pipe_cmd_valid=1, bram_do still has the OLD data.
    -- We need to delay the capture by one cycle using pipe_lookup_d1.
    --
    -- Timeline:
    --   Cycle 0: cmd_valid=1, cmd_addr=X
    --   Cycle 1: bram_addr=X, pipe_cmd_valid=1, bram_do=OLD (wrong!)
    --   Cycle 2: bram_do=bram(X) (correct!), pipe_lookup_d1=1, CAPTURE HERE
    --   Cycle 3: rd_valid_stage2=1, rd_data_stage2=bram(X)
    ------------------------------------------------------------------------
    process(clk)
    begin
        if rising_edge(clk) then
            if rst = '1' then
                ...
                -- Delay the LOOKUP flag by one cycle to align with valid bram_do
                ...
                -- Pipeline stage 1: Capture BRAM output when delayed LOOKUP flag is set
                -- At this point, bram_do contains the correct data for the requested address
                if pipe_lookup_d1 = '1' then
